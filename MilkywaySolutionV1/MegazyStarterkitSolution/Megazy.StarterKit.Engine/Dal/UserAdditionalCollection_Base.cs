...
namespace Megazy.StarterKit.Engine.Dal
{
	...
	public partial class UserAdditionalCollection_Base : MarshalByRefObject
	{
		...
		public virtual UserAdditionalRow[] GetByDepartmentID(int departmentId)
		{
		...
		string sql = "SELECT COUNT(UserID) AS TotalRow FROM [dbo].[UserAdditional] " + whereCount;
		SqlCommand command = CreateCommand(sql);
		command.Parameters.AddRange(sqlParameter.ToArray());
		...
		sql = " SELECT RowRank,UserID,DepartmentID,IsAdministrator,ModifiedDate," + totalRow + " AS totalRow," + totalPage + " AS totalPage " +
		" FROM (SELECT *, " +
