...
namespace Megazy.StarterKit.Engine.Dal
{
	...
	public partial class UserCollection_Base : MarshalByRefObject
	{
		...
		sql = " SELECT RowRank,UserID,UserName,AliasName,Password,FirstName,LastName,Gender,PhoneNumber,Email,DateOfBirth,RegistDate,UpdateDate,IsVerifyByEmail,IsRenewAccount,UserStatusID,LastLoginDate,UserTypeID,UpdateBy,RenewTicketID," + totalRow + " AS totalRow," + totalPage + " AS totalPage " +
		" FROM (SELECT [User].*, " +
		...
		public int Insert(UserRow value)		{
			...
			CreateSqlParameterName("RenewTicketID") + "); SELECT SCOPE_IDENTITY();";
