...
{
	public partial class View_ApplicationCollection_Base
	{
		...
		public const string DateKPIStartColumnName = "DateKPIStart";
		public const string DateKPIEndColumnName = "DateKPIEnd";
		public const string LastKPIDayColumnName = "LastKPIDay";
		private int _processID;
		public SqlCommand cmd = null;
		public View_ApplicationCollection_Base(int intProcessID)
		{
		...
		public virtual View_ApplicationRow[] GetAll()
		{
		...
		public  DataTable CreateDataTable()
		{
			...
			dataColumn.AllowDBNull = false;
			dataColumn = dataTable.Columns.Add("DateKPIStart",Type.GetType("System.DateTime"));
			dataColumn = dataTable.Columns.Add("DateKPIEnd",Type.GetType("System.DateTime"));
			dataColumn = dataTable.Columns.Add("LastKPIDay",Type.GetType("System.Int32"));
			dataColumn.AllowDBNull = false;
			return dataTable;
		}
		public View_ApplicationRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql)
		{
			int totalRecordCount = -1;
			return GetAsArray(sqlParameter, whereSql, orderBySql, 0, int.MaxValue, ref totalRecordCount);
		}
		public virtual View_ApplicationRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql, int startIndex, int length, ref int totalRecordCount)
		{
		...
		/// <summary>
		/// Gets an array of <see cref="View_ApplicationRow"/> objects that
		...
		/// <returns>An array of <see cref="View_ApplicationRow"/> objects.</returns>
		public virtual View_ApplicationRow[] GetTopAsArray(int top,List<SqlParameter> sqlParameter,string whereSql, string orderBySql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			if (top <= 0)
				top = 1;
			string sql = string.Format("SELECT TOP ({0}) * FROM [dbo].[View_Application]", top);
			if (null != whereSql && 0 < whereSql.Length)
				sql += " WHERE " + whereSql;
			if (null != orderBySql && 0 < orderBySql.Length)
				sql += " ORDER BY " + orderBySql;
		...
		public View_ApplicationRow GetRow(List<SqlParameter> sqlParameter,string whereSql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			View_ApplicationRow[] rows = null;
		...
		/// <returns></returns>
		public DataTable GetView_ApplicationPagingAsDataTable(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "CaseID")
		{
			AntiSqlInjection.CheckInput(whereSql);
		if (startRowIndex < 0)
			startRowIndex = 0;
		...
		string sql = "SELECT COUNT(CaseID) AS TotalRow FROM [dbo].[View_Application] " + whereCount;
		SqlCommand command = CreateCommand(sql);
		command.Parameters.AddRange(sqlParameter.ToArray());
		...
		/// <param name="orderBySql"></param>
		/// <returns></returns>
		public View_ApplicationItemsPaging GetView_ApplicationPagingAsArray(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "CaseID")
		{
		View_ApplicationItemsPaging obj = new View_ApplicationItemsPaging();
		DataTable dt = GetView_ApplicationPagingAsDataTable(sqlParameter,whereSql,startRowIndex,rowPerPage,orderBySql);
		if(dt.Rows.Count !=0){
			obj.totalRow = Convert.ToInt32(dt.Rows[0]["totalRow"]);
			obj.totalPage = Convert.ToInt32(dt.Rows[0]["totalPage"]);
		}
		View_ApplicationItems record;
		ArrayList recordList = new ArrayList();
		for (int i = 0; i < dt.Rows.Count; i++)
		{
			record = new View_ApplicationItems();
			...
			if (dt.Rows[i]["DateKPIStart"] != DBNull.Value)
			record.DateKPIStart = Convert.ToDateTime(dt.Rows[i]["DateKPIStart"]);
			if (dt.Rows[i]["DateKPIEnd"] != DBNull.Value)
			record.DateKPIEnd = Convert.ToDateTime(dt.Rows[i]["DateKPIEnd"]);
			record.LastKPIDay = Convert.ToInt32(dt.Rows[i]["LastKPIDay"]);
			recordList.Add(record);
		}
		obj.view_ApplicationItems = (View_ApplicationItems[])(recordList.ToArray(typeof(View_ApplicationItems)));
		return obj;
		}
		protected View_ApplicationRow[] MapRecords(IDbCommand command)
		{
			using (IDataReader reader = ExecuteReader(command))
			{
		...
		protected View_ApplicationRow[] MapRecords(IDataReader reader)		{
			int totalRecordCount = -1;
			return MapRecords(reader, 0, int.MaxValue, ref totalRecordCount);
		}
		protected View_ApplicationRow[] MapRecords(IDataReader reader, int startIndex, int length, ref int totalRecordCount)		{
			if (0 > startIndex)
			...
			int dateKPIStartColumnIndex = reader.GetOrdinal("DateKPIStart");
			int dateKPIEndColumnIndex = reader.GetOrdinal("DateKPIEnd");
			int lastKPIDayColumnIndex = reader.GetOrdinal("LastKPIDay");
			...
			while (reader.Read())
			{
				...
				if (ri > 0 && ri <= length)
				{
					View_ApplicationRow record = new View_ApplicationRow();
					recordList.Add(record);
					...
					if (!reader.IsDBNull(dateKPIStartColumnIndex)) record.DateKPIStart =  Convert.ToDateTime(reader.GetValue(dateKPIStartColumnIndex));

					if (!reader.IsDBNull(dateKPIEndColumnIndex)) record.DateKPIEnd =  Convert.ToDateTime(reader.GetValue(dateKPIEndColumnIndex));

					record.LastKPIDay =  Convert.ToInt32(reader.GetValue(lastKPIDayColumnIndex));
			...
			return (View_ApplicationRow[])(recordList.ToArray(typeof(View_ApplicationRow)));
		}
