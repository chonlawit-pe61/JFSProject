...
{
	[Serializable]
	public partial class View_LawyerOfficeAddressCollection_Base : MarshalByRefObject
	{
		...
		public View_LawyerOfficeAddressCollection_Base(int intProcessID)
		{
		...
		public virtual View_LawyerOfficeAddressRow[] GetAll()
		{
		...
		protected virtual SqlCommand CreateGetCommand(string whereSql, string orderBySql)
		{
			...
			"[SubdistrictCode],"+
			"[Soi]"+
			" FROM [dbo].[View_LawyerOfficeAddress]";
		...
		public  DataTable CreateDataTable()
		{
			...
			{
				TableName = "View_LawyerOfficeAddress"
			};
		...
		public View_LawyerOfficeAddressRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql)
		{
			int totalRecordCount = -1;
			return GetAsArray(sqlParameter, whereSql, orderBySql, 0, int.MaxValue, ref totalRecordCount);
		}
		public virtual View_LawyerOfficeAddressRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql, int startIndex, int length, ref int totalRecordCount)
		{
		...
		/// <summary>
		/// Gets an array of <see cref="View_LawyerOfficeAddressRow"/> objects that
		...
		/// <returns>An array of <see cref="View_LawyerOfficeAddressRow"/> objects.</returns>
		public virtual View_LawyerOfficeAddressRow[] GetTopAsArray(int top,List<SqlParameter> sqlParameter,string whereSql, string orderBySql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			if (top <= 0)
				top = 1;
			string sql = string.Format("SELECT TOP ({0}) * FROM [dbo].[View_LawyerOfficeAddress]", top);
			if (null != whereSql && 0 < whereSql.Length)
				sql += " WHERE " + whereSql;
			if (null != orderBySql && 0 < orderBySql.Length)
				sql += " ORDER BY " + orderBySql;
		...
		public View_LawyerOfficeAddressRow GetRow(List<SqlParameter> sqlParameter,string whereSql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			View_LawyerOfficeAddressRow[] rows = null;
		...
		/// <returns></returns>
		public DataTable GetView_LawyerOfficeAddressPagingAsDataTable(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "LawyerOfficeID")
		{
			AntiSqlInjection.CheckInput(whereSql);
		if (startRowIndex < 0)
			startRowIndex = 0;
		...
		string sql = "SELECT COUNT(1) AS TotalRow FROM [dbo].[View_LawyerOfficeAddress] " + whereCount;
		SqlCommand command = CreateCommand(sql);
		command.Parameters.AddRange(sqlParameter.ToArray());
		...
		sql = " SELECT RowRank,LawyerOfficeID,AddressID,Address1,HouseNo,VillageNo,Street,ProvinceID,DisctrictID,SubdistrictID,PostCode,ProvinceName,ProvinceCode,CountryCode,DistrictName,DistrictCode,SubdistrictName,SubdistrictCode,Soi," + totalRow + " AS totalRow," + totalPage + " AS totalPage " +
		" FROM (SELECT [View_LawyerOfficeAddress].*, " +
		" ROW_NUMBER() OVER(ORDER BY " + orderBySql + ") AS RowRank " +
		" FROM [dbo].[View_LawyerOfficeAddress] " + whereSql +
		" ) AS AllWithRowRank " +
		...
		/// <param name="orderBySql"></param>
		/// <returns></returns>
		public View_LawyerOfficeAddressItemsPaging GetView_LawyerOfficeAddressPagingAsArray(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "LawyerOfficeID")
		{
		View_LawyerOfficeAddressItemsPaging obj = new View_LawyerOfficeAddressItemsPaging();
		DataTable dt = GetView_LawyerOfficeAddressPagingAsDataTable(sqlParameter,whereSql,startRowIndex,rowPerPage,orderBySql);
		if(dt.Rows.Count !=0){
			obj.totalRow = Convert.ToInt32(dt.Rows[0]["totalRow"]);
			obj.totalPage = Convert.ToInt32(dt.Rows[0]["totalPage"]);
		}
		View_LawyerOfficeAddressItems record;
		ArrayList recordList = new ArrayList();
		for (int i = 0; i < dt.Rows.Count; i++)
		{
			record = new View_LawyerOfficeAddressItems();
		...
		obj.view_LawyerOfficeAddressItems = (View_LawyerOfficeAddressItems[])(recordList.ToArray(typeof(View_LawyerOfficeAddressItems)));
		return obj;
		}
		protected View_LawyerOfficeAddressRow[] MapRecords(IDbCommand command)
		{
			using (IDataReader reader = ExecuteReader(command))
			{
		...
		protected View_LawyerOfficeAddressRow[] MapRecords(IDataReader reader)		{
			int totalRecordCount = -1;
			return MapRecords(reader, 0, int.MaxValue, ref totalRecordCount);
		}
		protected View_LawyerOfficeAddressRow[] MapRecords(IDataReader reader, int startIndex, int length, ref int totalRecordCount)		{
			if (0 > startIndex)
			...
			while (reader.Read())
			{
				...
				if (ri > 0 && ri <= length)
				{
					View_LawyerOfficeAddressRow record = new View_LawyerOfficeAddressRow();
					recordList.Add(record);
			...
			return (View_LawyerOfficeAddressRow[])(recordList.ToArray(typeof(View_LawyerOfficeAddressRow)));
		}
