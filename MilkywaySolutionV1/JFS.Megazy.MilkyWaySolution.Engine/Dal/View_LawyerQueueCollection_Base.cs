...
{
	[Serializable]
	public partial class View_LawyerQueueCollection_Base : MarshalByRefObject
	{
		...
		public View_LawyerQueueCollection_Base(int intProcessID)
		{
		...
		public virtual View_LawyerQueueRow[] GetAll()
		{
		...
		public View_LawyerQueueRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql)
		{
			int totalRecordCount = -1;
			return GetAsArray(sqlParameter, whereSql, orderBySql, 0, int.MaxValue, ref totalRecordCount);
		}
		public virtual View_LawyerQueueRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql, int startIndex, int length, ref int totalRecordCount)
		{
		...
		/// <summary>
		/// Gets an array of <see cref="View_LawyerQueueRow"/> objects that
		...
		/// <returns>An array of <see cref="View_LawyerQueueRow"/> objects.</returns>
		public virtual View_LawyerQueueRow[] GetTopAsArray(int top,List<SqlParameter> sqlParameter,string whereSql, string orderBySql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			if (top <= 0)
				top = 1;
			string sql = string.Format("SELECT TOP ({0}) * FROM [dbo].[View_LawyerQueue]", top);
			if (null != whereSql && 0 < whereSql.Length)
				sql += " WHERE " + whereSql;
			if (null != orderBySql && 0 < orderBySql.Length)
				sql += " ORDER BY " + orderBySql;
		...
		public View_LawyerQueueRow GetRow(List<SqlParameter> sqlParameter,string whereSql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			View_LawyerQueueRow[] rows = null;
		...
		/// <returns></returns>
		public DataTable GetView_LawyerQueuePagingAsDataTable(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "LawyerID")
		{
			AntiSqlInjection.CheckInput(whereSql);
		if (startRowIndex < 0)
			startRowIndex = 0;
		...
		string sql = "SELECT COUNT(1) AS TotalRow FROM [dbo].[View_LawyerQueue] " + whereCount;
		SqlCommand command = CreateCommand(sql);
		command.Parameters.AddRange(sqlParameter.ToArray());
		...
		/// <param name="orderBySql"></param>
		/// <returns></returns>
		public View_LawyerQueueItemsPaging GetView_LawyerQueuePagingAsArray(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "LawyerID")
		{
		View_LawyerQueueItemsPaging obj = new View_LawyerQueueItemsPaging();
		DataTable dt = GetView_LawyerQueuePagingAsDataTable(sqlParameter,whereSql,startRowIndex,rowPerPage,orderBySql);
		if(dt.Rows.Count !=0){
			obj.totalRow = Convert.ToInt32(dt.Rows[0]["totalRow"]);
			obj.totalPage = Convert.ToInt32(dt.Rows[0]["totalPage"]);
		}
		View_LawyerQueueItems record;
		ArrayList recordList = new ArrayList();
		for (int i = 0; i < dt.Rows.Count; i++)
		{
			record = new View_LawyerQueueItems();
			...
			if (dt.Rows[i]["QueueID"] != DBNull.Value)
			...
			if (dt.Rows[i]["QueueStatusID"] != DBNull.Value)
			...
			if (dt.Rows[i]["Priority"] != DBNull.Value)
		...
		obj.view_LawyerQueueItems = (View_LawyerQueueItems[])(recordList.ToArray(typeof(View_LawyerQueueItems)));
		return obj;
		}
		public View_LawyerQueueRow[] GetIsActive(bool isActive)
		{
			string whereSql = "[IsActive]=" + CreateSqlParameterName("IsActive");
			IDbCommand cmd = CreateGetCommand(whereSql, null);
			AddParameter(cmd, "IsActive", isActive);
			View_LawyerQueueRow[] tempArray = MapRecords(cmd);
			return 0 == tempArray.Length ? null : tempArray;
		}
		protected View_LawyerQueueRow[] MapRecords(IDbCommand command)
		{
			using (IDataReader reader = ExecuteReader(command))
			{
		...
		protected View_LawyerQueueRow[] MapRecords(IDataReader reader)		{
			int totalRecordCount = -1;
			return MapRecords(reader, 0, int.MaxValue, ref totalRecordCount);
		}
		protected View_LawyerQueueRow[] MapRecords(IDataReader reader, int startIndex, int length, ref int totalRecordCount)		{
			if (0 > startIndex)
			...
			while (reader.Read())
			{
				...
				if (ri > 0 && ri <= length)
				{
					View_LawyerQueueRow record = new View_LawyerQueueRow();
					recordList.Add(record);
					...
					if (!reader.IsDBNull(queueIDColumnIndex)) record.QueueID =  Convert.ToInt32(reader.GetValue(queueIDColumnIndex));
					...
					if (!reader.IsDBNull(queueStatusIDColumnIndex)) record.QueueStatusID =  Convert.ToInt32(reader.GetValue(queueStatusIDColumnIndex));
					...
					if (!reader.IsDBNull(priorityColumnIndex)) record.Priority =  Convert.ToInt32(reader.GetValue(priorityColumnIndex));
			...
			return (View_LawyerQueueRow[])(recordList.ToArray(typeof(View_LawyerQueueRow)));
		}
