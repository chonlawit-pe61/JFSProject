...
{
	[Serializable]
	public partial class OfficerOpinionCollection_Base : MarshalByRefObject
	{
		...
		public OfficerOpinionCollection_Base(int intProcessID)
		{
		...
		public virtual OfficerOpinionRow[] GetAll()
		{
		...
		{
			AntiSqlInjection.CheckInput(whereSql);
			string sql = "DELETE FROM [dbo].[OfficerOpinion]";
			if (null != whereSql && 0 < whereSql.Length)
			sql += " WHERE " + whereSql;
			return CreateCommand(sql);
		}
		...
		public virtual OfficerOpinionRow[] GetByOfficerRoleID(int officerRoleID)
		{
		...
		public OfficerOpinionRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql)
		{
			int totalRecordCount = -1;
			return GetAsArray(sqlParameter, whereSql, orderBySql, 0, int.MaxValue, ref totalRecordCount);
		}
		public virtual OfficerOpinionRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql, int startIndex, int length, ref int totalRecordCount)
		{
		...
		/// <summary>
		/// Gets an array of <see cref="OfficerOpinionRow"/> objects that
		...
		/// <returns>An array of <see cref="OfficerOpinionRow"/> objects.</returns>
		public virtual OfficerOpinionRow[] GetTopAsArray(int top,List<SqlParameter> sqlParameter,string whereSql, string orderBySql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			if (top <= 0)
				top = 1;
			string sql = string.Format("SELECT TOP ({0}) * FROM [dbo].[OfficerOpinion]", top);
			if (null != whereSql && 0 < whereSql.Length)
				sql += " WHERE " + whereSql;
			if (null != orderBySql && 0 < orderBySql.Length)
				sql += " ORDER BY " + orderBySql;
		...
		public OfficerOpinionRow GetRow(List<SqlParameter> sqlParameter,string whereSql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			OfficerOpinionRow[] rows = null;
		...
		/// <returns></returns>
		public DataTable GetOfficerOpinionPagingAsDataTable(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "OpinionID")
		{
			AntiSqlInjection.CheckInput(whereSql);
		if (startRowIndex < 0)
			startRowIndex = 0;
		...
		string sql = "SELECT COUNT(OpinionID) AS TotalRow FROM [dbo].[OfficerOpinion] " + whereCount;
		SqlCommand command = CreateCommand(sql);
		command.Parameters.AddRange(sqlParameter.ToArray());
		...
		sql = " SELECT RowRank,OpinionID,JFCaseTypeID,OfficerRoleID,SortOrder,IsActive,ModifiedDate," + totalRow + " AS totalRow," + totalPage + " AS totalPage " +
		" FROM (SELECT *, " +
		" ROW_NUMBER() OVER(ORDER BY " + orderBySql + ") AS RowRank " +
		" FROM [dbo].[OfficerOpinion] " + whereSql +
		" ) AS AllWithRowRank " +
		...
		/// <param name="orderBySql"></param>
		/// <returns></returns>
		public OfficerOpinionItemsPaging GetOfficerOpinionPagingAsArray(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "OpinionID")
		{
		OfficerOpinionItemsPaging obj = new OfficerOpinionItemsPaging();
		DataTable dt = GetOfficerOpinionPagingAsDataTable(sqlParameter,whereSql,startRowIndex,rowPerPage,orderBySql);
		if(dt.Rows.Count !=0){
			obj.totalRow = Convert.ToInt32(dt.Rows[0]["totalRow"]);
			obj.totalPage = Convert.ToInt32(dt.Rows[0]["totalPage"]);
		}
		OfficerOpinionItems record;
		ArrayList recordList = new ArrayList();
		for (int i = 0; i < dt.Rows.Count; i++)
		{
			record = new OfficerOpinionItems();
		...
		obj.officeropinionItems = (OfficerOpinionItems[])(recordList.ToArray(typeof(OfficerOpinionItems)));
		return obj;
		}
		public OfficerOpinionRow[] GetIsActive(bool isActive)
		{
			string whereSql = "[IsActive]=" + CreateSqlParameterName("IsActive");
			IDbCommand cmd = CreateGetCommand(whereSql, null);
			AddParameter(cmd, "IsActive", isActive);
			OfficerOpinionRow[] tempArray = MapRecords(cmd);
		...
		protected OfficerOpinionRow[] MapRecords(IDbCommand command)
		{
			using (IDataReader reader = ExecuteReader(command))
			{
		...
		protected OfficerOpinionRow[] MapRecords(IDataReader reader)		{
			int totalRecordCount = -1;
			return MapRecords(reader, 0, int.MaxValue, ref totalRecordCount);
		}
		protected OfficerOpinionRow[] MapRecords(IDataReader reader, int startIndex, int length, ref int totalRecordCount)		{
			if (0 > startIndex)
			...
			while (reader.Read())
			{
				...
				if (ri > 0 && ri <= length)
				{
					OfficerOpinionRow record = new OfficerOpinionRow();
					recordList.Add(record);
			...
			return (OfficerOpinionRow[])(recordList.ToArray(typeof(OfficerOpinionRow)));
		}
