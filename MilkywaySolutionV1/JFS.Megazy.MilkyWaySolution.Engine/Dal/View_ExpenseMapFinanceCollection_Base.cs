...
{
	[Serializable]
	public partial class View_ExpenseMapFinanceCollection_Base : MarshalByRefObject
	{
		...
		public View_ExpenseMapFinanceCollection_Base(int intProcessID)
		{
		...
		public virtual View_ExpenseMapFinanceRow[] GetAll()
		{
		...
		protected virtual SqlCommand CreateGetCommand(string whereSql, string orderBySql)
		{
			...
			"[CaseTypeNameAbbr],"+
			"[CaseTypePrefix]"+
			" FROM [dbo].[View_ExpenseMapFinance]";
		...
		public  DataTable CreateDataTable()
		{
			...
			{
				TableName = "View_ExpenseMapFinance"
			};
		...
		public View_ExpenseMapFinanceRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql)
		{
			int totalRecordCount = -1;
			return GetAsArray(sqlParameter, whereSql, orderBySql, 0, int.MaxValue, ref totalRecordCount);
		}
		public virtual View_ExpenseMapFinanceRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql, int startIndex, int length, ref int totalRecordCount)
		{
		...
		/// <summary>
		/// Gets an array of <see cref="View_ExpenseMapFinanceRow"/> objects that
		...
		/// <returns>An array of <see cref="View_ExpenseMapFinanceRow"/> objects.</returns>
		public virtual View_ExpenseMapFinanceRow[] GetTopAsArray(int top,List<SqlParameter> sqlParameter,string whereSql, string orderBySql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			if (top <= 0)
				top = 1;
			string sql = string.Format("SELECT TOP ({0}) * FROM [dbo].[View_ExpenseMapFinance]", top);
			if (null != whereSql && 0 < whereSql.Length)
				sql += " WHERE " + whereSql;
			if (null != orderBySql && 0 < orderBySql.Length)
				sql += " ORDER BY " + orderBySql;
		...
		public View_ExpenseMapFinanceRow GetRow(List<SqlParameter> sqlParameter,string whereSql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			View_ExpenseMapFinanceRow[] rows = null;
		...
		/// <returns></returns>
		public DataTable GetView_ExpenseMapFinancePagingAsDataTable(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "ExpenseID")
		{
			AntiSqlInjection.CheckInput(whereSql);
		if (startRowIndex < 0)
			startRowIndex = 0;
		...
		string sql = "SELECT COUNT(1) AS TotalRow FROM [dbo].[View_ExpenseMapFinance] " + whereCount;
		SqlCommand command = CreateCommand(sql);
		command.Parameters.AddRange(sqlParameter.ToArray());
		...
		sql = " SELECT RowRank,ExpenseID,JFCaseTypeID,IsOther,SortOrder,ExpenseName,CaseTypeName,CaseTypeNameAbbr,CaseTypePrefix," + totalRow + " AS totalRow," + totalPage + " AS totalPage " +
		" FROM (SELECT [View_ExpenseMapFinance].*, " +
		" ROW_NUMBER() OVER(ORDER BY " + orderBySql + ") AS RowRank " +
		" FROM [dbo].[View_ExpenseMapFinance] " + whereSql +
		" ) AS AllWithRowRank " +
		...
		/// <param name="orderBySql"></param>
		/// <returns></returns>
		public View_ExpenseMapFinanceItemsPaging GetView_ExpenseMapFinancePagingAsArray(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "ExpenseID")
		{
		View_ExpenseMapFinanceItemsPaging obj = new View_ExpenseMapFinanceItemsPaging();
		DataTable dt = GetView_ExpenseMapFinancePagingAsDataTable(sqlParameter,whereSql,startRowIndex,rowPerPage,orderBySql);
		if(dt.Rows.Count !=0){
			obj.totalRow = Convert.ToInt32(dt.Rows[0]["totalRow"]);
			obj.totalPage = Convert.ToInt32(dt.Rows[0]["totalPage"]);
		}
		View_ExpenseMapFinanceItems record;
		ArrayList recordList = new ArrayList();
		for (int i = 0; i < dt.Rows.Count; i++)
		{
			record = new View_ExpenseMapFinanceItems();
		...
		obj.view_ExpenseMapFinanceItems = (View_ExpenseMapFinanceItems[])(recordList.ToArray(typeof(View_ExpenseMapFinanceItems)));
		return obj;
		}
		protected View_ExpenseMapFinanceRow[] MapRecords(IDbCommand command)
		{
			using (IDataReader reader = ExecuteReader(command))
			{
		...
		protected View_ExpenseMapFinanceRow[] MapRecords(IDataReader reader)		{
			int totalRecordCount = -1;
			return MapRecords(reader, 0, int.MaxValue, ref totalRecordCount);
		}
		protected View_ExpenseMapFinanceRow[] MapRecords(IDataReader reader, int startIndex, int length, ref int totalRecordCount)		{
			if (0 > startIndex)
			...
			while (reader.Read())
			{
				...
				if (ri > 0 && ri <= length)
				{
					View_ExpenseMapFinanceRow record = new View_ExpenseMapFinanceRow();
					recordList.Add(record);
			...
			return (View_ExpenseMapFinanceRow[])(recordList.ToArray(typeof(View_ExpenseMapFinanceRow)));
		}
