...
{
	public partial class View_CaseApplicationMemberMappingCollection_Base
	{
		...
		public View_CaseApplicationMemberMappingCollection_Base(int intProcessID)
		{
		...
		public virtual View_CaseApplicationMemberMappingRow[] GetAll()
		{
		...
		protected virtual SqlCommand CreateGetCommand(string whereSql, string orderBySql)
		{
			...
			"[JFCaseNo],"+
			"[DateCreatedCaseNo]"+
			" FROM [dbo].[View_CaseApplicationMemberMapping]";
		...
		public  DataTable CreateDataTable()
		{
			...
			{
				TableName = "View_CaseApplicationMemberMapping"
			};
		...
		public View_CaseApplicationMemberMappingRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql)
		{
			int totalRecordCount = -1;
			return GetAsArray(sqlParameter, whereSql, orderBySql, 0, int.MaxValue, ref totalRecordCount);
		}
		public virtual View_CaseApplicationMemberMappingRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql, int startIndex, int length, ref int totalRecordCount)
		{
		...
		/// <summary>
		/// Gets an array of <see cref="View_CaseApplicationMemberMappingRow"/> objects that
		...
		/// <returns>An array of <see cref="View_CaseApplicationMemberMappingRow"/> objects.</returns>
		public virtual View_CaseApplicationMemberMappingRow[] GetTopAsArray(int top,List<SqlParameter> sqlParameter,string whereSql, string orderBySql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			if (top <= 0)
				top = 1;
			string sql = string.Format("SELECT TOP ({0}) * FROM [dbo].[View_CaseApplicationMemberMapping]", top);
			if (null != whereSql && 0 < whereSql.Length)
				sql += " WHERE " + whereSql;
			if (null != orderBySql && 0 < orderBySql.Length)
				sql += " ORDER BY " + orderBySql;
		...
		public View_CaseApplicationMemberMappingRow GetRow(List<SqlParameter> sqlParameter,string whereSql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			View_CaseApplicationMemberMappingRow[] rows = null;
		...
		/// <returns></returns>
		public DataTable GetView_CaseApplicationMemberMappingPagingAsDataTable(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "CaseID")
		{
			AntiSqlInjection.CheckInput(whereSql);
		if (startRowIndex < 0)
			startRowIndex = 0;
		...
		string sql = "SELECT COUNT(CaseID) AS TotalRow FROM [dbo].[View_CaseApplicationMemberMapping] " + whereCount;
		SqlCommand command = CreateCommand(sql);
		command.Parameters.AddRange(sqlParameter.ToArray());
		...
		sql = " SELECT RowRank,CaseID,ApplicantID,DepartmentID,ReferenceMSCID,Subject,Title,FirstName,LastName,CreateDate,MemberID,MemberFirstName,MemberLastName,OrgName,PhoneNumber,WorkStepID,WorkStepName,DeletedFlag,JFCaseNo,DateCreatedCaseNo," + totalRow + " AS totalRow," + totalPage + " AS totalPage " +
		" FROM (SELECT *, " +
		" ROW_NUMBER() OVER(ORDER BY " + orderBySql + ") AS RowRank " +
		" FROM [dbo].[View_CaseApplicationMemberMapping] " + whereSql +
		" ) AS AllWithRowRank " +
		...
		/// <param name="orderBySql"></param>
		/// <returns></returns>
		public View_CaseApplicationMemberMappingItemsPaging GetView_CaseApplicationMemberMappingPagingAsArray(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "CaseID")
		{
		View_CaseApplicationMemberMappingItemsPaging obj = new View_CaseApplicationMemberMappingItemsPaging();
		DataTable dt = GetView_CaseApplicationMemberMappingPagingAsDataTable(sqlParameter,whereSql,startRowIndex,rowPerPage,orderBySql);
		if(dt.Rows.Count !=0){
			obj.totalRow = Convert.ToInt32(dt.Rows[0]["totalRow"]);
			obj.totalPage = Convert.ToInt32(dt.Rows[0]["totalPage"]);
		}
		View_CaseApplicationMemberMappingItems record;
		ArrayList recordList = new ArrayList();
		for (int i = 0; i < dt.Rows.Count; i++)
		{
			record = new View_CaseApplicationMemberMappingItems();
		...
		obj.view_CaseApplicationMemberMappingItems = (View_CaseApplicationMemberMappingItems[])(recordList.ToArray(typeof(View_CaseApplicationMemberMappingItems)));
		return obj;
		}
		protected View_CaseApplicationMemberMappingRow[] MapRecords(IDbCommand command)
		{
			using (IDataReader reader = ExecuteReader(command))
			{
		...
		protected View_CaseApplicationMemberMappingRow[] MapRecords(IDataReader reader)		{
			int totalRecordCount = -1;
			return MapRecords(reader, 0, int.MaxValue, ref totalRecordCount);
		}
		protected View_CaseApplicationMemberMappingRow[] MapRecords(IDataReader reader, int startIndex, int length, ref int totalRecordCount)		{
			if (0 > startIndex)
			...
			while (reader.Read())
			{
				...
				if (ri > 0 && ri <= length)
				{
					View_CaseApplicationMemberMappingRow record = new View_CaseApplicationMemberMappingRow();
					recordList.Add(record);
			...
			return (View_CaseApplicationMemberMappingRow[])(recordList.ToArray(typeof(View_CaseApplicationMemberMappingRow)));
		}
