...
{
	[Serializable]
	public partial class View_LawyerTerritoryCollection_Base : MarshalByRefObject
	{
		...
		public const string NumberOfContractColumnName = "NumberOfContract";
		public const string TerritoryProvinceIDColumnName = "TerritoryProvinceID";
		public const string TeritoryProvinceNameColumnName = "TeritoryProvinceName";
		public const string ContractDateColumnName = "ContractDate";
		public const string CaseTypeColumnName = "CaseType";
		private int _processID;
		public SqlCommand cmd = null;
		public View_LawyerTerritoryCollection_Base(int intProcessID)
		{
		...
		public virtual View_LawyerTerritoryRow[] GetAll()
		{
		...
		protected virtual SqlCommand CreateGetCommand(string whereSql, string orderBySql)
		{
			...
			"[NumberOfContract],"+
			"[TerritoryProvinceID],"+
			"[TeritoryProvinceName],"+
			"[ContractDate],"+
			"[CaseType]"+
			" FROM [dbo].[View_LawyerTerritory]";
		...
		public  DataTable CreateDataTable()
		{
			...
			{
				TableName = "View_LawyerTerritory"
			};
			...
			dataColumn.MaxLength = 50;
			dataColumn.AllowDBNull = false;
			dataColumn = dataTable.Columns.Add("NumberOfContract",Type.GetType("System.Int32"));
			dataColumn.AllowDBNull = false;
			dataColumn = dataTable.Columns.Add("TerritoryProvinceID",Type.GetType("System.Int32"));
			dataColumn.AllowDBNull = false;
			dataColumn = dataTable.Columns.Add("TeritoryProvinceName",Type.GetType("System.String"));
			dataColumn.MaxLength = 150;
			dataColumn = dataTable.Columns.Add("ContractDate",Type.GetType("System.DateTime"));
			dataColumn = dataTable.Columns.Add("CaseType",Type.GetType("System.String"));
			dataColumn.MaxLength = 2147483647;
			return dataTable;
		}
		public View_LawyerTerritoryRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql)
		{
			int totalRecordCount = -1;
			return GetAsArray(sqlParameter, whereSql, orderBySql, 0, int.MaxValue, ref totalRecordCount);
		}
		public virtual View_LawyerTerritoryRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql, int startIndex, int length, ref int totalRecordCount)
		{
		...
		/// <summary>
		/// Gets an array of <see cref="View_LawyerTerritoryRow"/> objects that
		...
		/// <returns>An array of <see cref="View_LawyerTerritoryRow"/> objects.</returns>
		public virtual View_LawyerTerritoryRow[] GetTopAsArray(int top,List<SqlParameter> sqlParameter,string whereSql, string orderBySql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			if (top <= 0)
				top = 1;
			string sql = string.Format("SELECT TOP ({0}) * FROM [dbo].[View_LawyerTerritory]", top);
			if (null != whereSql && 0 < whereSql.Length)
				sql += " WHERE " + whereSql;
			if (null != orderBySql && 0 < orderBySql.Length)
				sql += " ORDER BY " + orderBySql;
		...
		public View_LawyerTerritoryRow GetRow(List<SqlParameter> sqlParameter,string whereSql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			View_LawyerTerritoryRow[] rows = null;
		...
		/// <returns></returns>
		public DataTable GetView_LawyerTerritoryPagingAsDataTable(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "LawyerID")
		{
			AntiSqlInjection.CheckInput(whereSql);
		if (startRowIndex < 0)
			startRowIndex = 0;
		...
		string sql = "SELECT COUNT(1) AS TotalRow FROM [dbo].[View_LawyerTerritory] " + whereCount;
		SqlCommand command = CreateCommand(sql);
		command.Parameters.AddRange(sqlParameter.ToArray());
		...
		sql = " SELECT RowRank,LawyerID,LawyerTypeID,Title,FirstName,LastName,CardID,Gender,LicenseNo,LicenseType,IssueDate,ExprieDate,Email,MobileNo,Education,Remark,NumberOfConductCase,YearsExperience,RegisterDate,DateOfBirth,ModifiedDate,LawyerStatusID,LawyerStatusName,AddressID,AddressTypeID,FaxNo,TelephoneNo,IsPrimary,IsActive,Address1,HouseNo,VillageNo,Street,ProvinceID,ProvinceName,LawyerProvinceName,DisctrictID,DistrictName,SubdistrictID,SubdistrictName,PostCode,LawyerOfficeID,LawyerFirmName,FirmelephoneNo,FirmFaxNo,FirmEmail,FirmIsActive,LawyerTypeName,NumberOfContract,TerritoryProvinceID,TeritoryProvinceName,ContractDate,CaseType," + totalRow + " AS totalRow," + totalPage + " AS totalPage " +
		" FROM (SELECT [View_LawyerTerritory].*, " +
		" ROW_NUMBER() OVER(ORDER BY " + orderBySql + ") AS RowRank " +
		" FROM [dbo].[View_LawyerTerritory] " + whereSql +
		" ) AS AllWithRowRank " +
		...
		/// <param name="orderBySql"></param>
		/// <returns></returns>
		public View_LawyerTerritoryItemsPaging GetView_LawyerTerritoryPagingAsArray(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "LawyerID")
		{
		View_LawyerTerritoryItemsPaging obj = new View_LawyerTerritoryItemsPaging();
		DataTable dt = GetView_LawyerTerritoryPagingAsDataTable(sqlParameter,whereSql,startRowIndex,rowPerPage,orderBySql);
		if(dt.Rows.Count !=0){
			obj.totalRow = Convert.ToInt32(dt.Rows[0]["totalRow"]);
			obj.totalPage = Convert.ToInt32(dt.Rows[0]["totalPage"]);
		}
		View_LawyerTerritoryItems record;
		ArrayList recordList = new ArrayList();
		for (int i = 0; i < dt.Rows.Count; i++)
		{
			record = new View_LawyerTerritoryItems();
			...
			record.NumberOfContract = Convert.ToInt32(dt.Rows[i]["NumberOfContract"]);
			record.TerritoryProvinceID = Convert.ToInt32(dt.Rows[i]["TerritoryProvinceID"]);
			record.TeritoryProvinceName = dt.Rows[i]["TeritoryProvinceName"].ToString();
			if (dt.Rows[i]["ContractDate"] != DBNull.Value)
			record.ContractDate = Convert.ToDateTime(dt.Rows[i]["ContractDate"]);
			record.CaseType = dt.Rows[i]["CaseType"].ToString();
			recordList.Add(record);
		}
		obj.view_LawyerTerritoryItems = (View_LawyerTerritoryItems[])(recordList.ToArray(typeof(View_LawyerTerritoryItems)));
		return obj;
		}
		public View_LawyerTerritoryRow[] GetIsActive(bool isActive)
		{
			string whereSql = "[IsActive]=" + CreateSqlParameterName("IsActive");
			IDbCommand cmd = CreateGetCommand(whereSql, null);
			AddParameter(cmd, "IsActive", isActive);
			View_LawyerTerritoryRow[] tempArray = MapRecords(cmd);
			return 0 == tempArray.Length ? null : tempArray;
		}
		protected View_LawyerTerritoryRow[] MapRecords(IDbCommand command)
		{
			using (IDataReader reader = ExecuteReader(command))
			{
		...
		protected View_LawyerTerritoryRow[] MapRecords(IDataReader reader)		{
			int totalRecordCount = -1;
			return MapRecords(reader, 0, int.MaxValue, ref totalRecordCount);
		}
		protected View_LawyerTerritoryRow[] MapRecords(IDataReader reader, int startIndex, int length, ref int totalRecordCount)		{
			if (0 > startIndex)
			...
			int numberOfContractColumnIndex = reader.GetOrdinal("NumberOfContract");
			int territoryProvinceIDColumnIndex = reader.GetOrdinal("TerritoryProvinceID");
			int teritoryProvinceNameColumnIndex = reader.GetOrdinal("TeritoryProvinceName");
			int contractDateColumnIndex = reader.GetOrdinal("ContractDate");
			int caseTypeColumnIndex = reader.GetOrdinal("CaseType");
			...
			while (reader.Read())
			{
				...
				if (ri > 0 && ri <= length)
				{
					View_LawyerTerritoryRow record = new View_LawyerTerritoryRow();
					recordList.Add(record);
					...
					record.NumberOfContract =  Convert.ToInt32(reader.GetValue(numberOfContractColumnIndex));
					record.TerritoryProvinceID =  Convert.ToInt32(reader.GetValue(territoryProvinceIDColumnIndex));
					if (!reader.IsDBNull(teritoryProvinceNameColumnIndex)) record.TeritoryProvinceName =  Convert.ToString(reader.GetValue(teritoryProvinceNameColumnIndex));
					...
					if (!reader.IsDBNull(caseTypeColumnIndex)) record.CaseType =  Convert.ToString(reader.GetValue(caseTypeColumnIndex));
			...
			return (View_LawyerTerritoryRow[])(recordList.ToArray(typeof(View_LawyerTerritoryRow)));
		}
		...
		protected virtual IDbDataParameter AddParameter(IDbCommand cmd, string paramName, object value)
		{
			...
			switch (paramName)
			{
				...
				case "LawyerTypeName":
					parameter = AddParameter(cmd, paramName, DbType.String, value);
					break;
				case "NumberOfContract":
					parameter = AddParameter(cmd, paramName, DbType.Int32, value);
					break;
				case "TerritoryProvinceID":
					parameter = AddParameter(cmd, paramName, DbType.Int32, value);
					break;
				case "TeritoryProvinceName":
					parameter = AddParameter(cmd, paramName, DbType.String, value);
					break;
				case "ContractDate":
					parameter = AddParameter(cmd, paramName, DbType.Date, value);
