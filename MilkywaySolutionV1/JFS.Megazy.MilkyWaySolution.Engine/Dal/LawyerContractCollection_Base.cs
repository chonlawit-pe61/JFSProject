...
{
	[Serializable]
	public partial class LawyerContractCollection_Base : MarshalByRefObject
	{
		...
		public LawyerContractCollection_Base(int intProcessID)
		{
		...
		public virtual LawyerContractRow[] GetAll()
		{
		...
		{
			AntiSqlInjection.CheckInput(whereSql);
			string sql = "DELETE FROM [dbo].[LawyerContract]";
			if (null != whereSql && 0 < whereSql.Length)
			sql += " WHERE " + whereSql;
			return CreateCommand(sql);
		}
		...
		public virtual LawyerContractRow[] GetByContractID(int contractID)
		{
		...
		public virtual LawyerContractRow[] GetByLawyerID(int lawyerID)
		{
		...
		public virtual LawyerContractRow[] GetByCourtJudgmentID(int courtJudgmentID)
		{
			return MapRecords(CreateGetByCourtJudgmentIDCommand(courtJudgmentID));
		}
		public virtual DataTable GetByCourtJudgmentIDAsDataTable(int courtJudgmentID)
		{
			return MapRecordsToDataTable(CreateGetByCourtJudgmentIDCommand(courtJudgmentID));
		}
		protected virtual IDbCommand CreateGetByCourtJudgmentIDCommand(int courtJudgmentID)
		{
			string whereSql = "";
			whereSql += "[CourtJudgmentID]=" + CreateSqlParameterName("CourtJudgmentID");
			IDbCommand cmd = CreateGetCommand(whereSql, null);
			AddParameter(cmd, "CourtJudgmentID", courtJudgmentID);
			return cmd;
		}
		public LawyerContractRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql)
		{
			int totalRecordCount = -1;
			return GetAsArray(sqlParameter, whereSql, orderBySql, 0, int.MaxValue, ref totalRecordCount);
		}
		public virtual LawyerContractRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql, int startIndex, int length, ref int totalRecordCount)
		{
		...
		/// <summary>
		/// Gets an array of <see cref="LawyerContractRow"/> objects that
		...
		/// <returns>An array of <see cref="LawyerContractRow"/> objects.</returns>
		public virtual LawyerContractRow[] GetTopAsArray(int top,List<SqlParameter> sqlParameter,string whereSql, string orderBySql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			if (top <= 0)
				top = 1;
			string sql = string.Format("SELECT TOP ({0}) * FROM [dbo].[LawyerContract]", top);
			if (null != whereSql && 0 < whereSql.Length)
				sql += " WHERE " + whereSql;
			if (null != orderBySql && 0 < orderBySql.Length)
				sql += " ORDER BY " + orderBySql;
		...
		public LawyerContractRow GetRow(List<SqlParameter> sqlParameter,string whereSql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			LawyerContractRow[] rows = null;
		...
		/// <returns></returns>
		public DataTable GetLawyerContractPagingAsDataTable(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "ContractID")
		{
			AntiSqlInjection.CheckInput(whereSql);
		if (startRowIndex < 0)
			startRowIndex = 0;
		...
		string sql = "SELECT COUNT(ContractID) AS TotalRow FROM [dbo].[LawyerContract] " + whereCount;
		SqlCommand command = CreateCommand(sql);
		command.Parameters.AddRange(sqlParameter.ToArray());
		...
		sql = " SELECT RowRank,ContractID,LawyerID,CourtJudgmentID,IsActive,Note,ModifiedDate," + totalRow + " AS totalRow," + totalPage + " AS totalPage " +
		" FROM (SELECT *, " +
		" ROW_NUMBER() OVER(ORDER BY " + orderBySql + ") AS RowRank " +
		" FROM [dbo].[LawyerContract] " + whereSql +
		" ) AS AllWithRowRank " +
		...
		/// <param name="orderBySql"></param>
		/// <returns></returns>
		public LawyerContractItemsPaging GetLawyerContractPagingAsArray(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "ContractID")
		{
		LawyerContractItemsPaging obj = new LawyerContractItemsPaging();
		DataTable dt = GetLawyerContractPagingAsDataTable(sqlParameter,whereSql,startRowIndex,rowPerPage,orderBySql);
		if(dt.Rows.Count !=0){
			obj.totalRow = Convert.ToInt32(dt.Rows[0]["totalRow"]);
			obj.totalPage = Convert.ToInt32(dt.Rows[0]["totalPage"]);
		}
		LawyerContractItems record;
		ArrayList recordList = new ArrayList();
		for (int i = 0; i < dt.Rows.Count; i++)
		{
			record = new LawyerContractItems();
		...
		obj.lawyerContractItems = (LawyerContractItems[])(recordList.ToArray(typeof(LawyerContractItems)));
		return obj;
		}
		public LawyerContractRow GetByPrimaryKey(int contractID)
		{
			string whereSql = "[ContractID]=" + CreateSqlParameterName("ContractID");
			IDbCommand cmd = CreateGetCommand(whereSql, null);
			AddParameter(cmd, "ContractID", contractID);
			LawyerContractRow[] tempArray = MapRecords(cmd);
			return 0 == tempArray.Length ? null : tempArray[0];
		}
		public void Insert(LawyerContractRow value)		{
			string sqlStr = "INSERT INTO [dbo].[LawyerContract] (" +
			"[ContractID], " + 
			"[LawyerID], " + 
			"[CourtJudgmentID], " + 
			"[IsActive], " + 
			...
			CreateSqlParameterName("CourtJudgmentID") + ", " +
			...
			AddParameter(cmd, "CourtJudgmentID", value.CourtJudgmentID);
		...
		public void InsertOnlyPlainText(LawyerContractRow value)		{
			string sqlStr = "INSERT INTO [dbo].[LawyerContract] (" +
			"[ContractID], " + 
			"[LawyerID], " + 
			"[CourtJudgmentID], " + 
		...
		public bool Update(LawyerContractRow value)
		{
			IDbCommand cmd = null;
			if ( value._IsSetContractID == true )
			{
				...
				if (value._IsSetCourtJudgmentID)
				{
					strUpdate += "[CourtJudgmentID]=" + CreateSqlParameterName("CourtJudgmentID") + ",";
				}
				...
				if (strUpdate != string.Empty)
				{
					...
					strUpdate = "UPDATE [dbo].[LawyerContract] SET " + strUpdate;
					strUpdate += " WHERE ";
		...
		public bool UpdateOnlyPlainText(LawyerContractRow value)
		{
			IDbCommand cmd = null;
			if ( value._IsSetContractID == true )
			{
		...
		public LawyerContractRow[] GetIsActive(bool isActive)
		{
		...
		public int DeleteByCourtJudgmentID(int courtJudgmentID)
		{
			return CreateDeleteByCourtJudgmentIDCommand(courtJudgmentID).ExecuteNonQuery();
		}
		protected virtual IDbCommand CreateDeleteByCourtJudgmentIDCommand(int courtJudgmentID)
		{
			string whereSql = "";
			whereSql += "[CourtJudgmentID]=" + CreateSqlParameterName("CourtJudgmentID");
			IDbCommand cmd = CreateDeleteCommand(whereSql);
		...
		protected LawyerContractRow[] MapRecords(IDbCommand command)
		{
			using (IDataReader reader = ExecuteReader(command))
			{
		...
		protected LawyerContractRow[] MapRecords(IDataReader reader)		{
			int totalRecordCount = -1;
			return MapRecords(reader, 0, int.MaxValue, ref totalRecordCount);
		}
		protected LawyerContractRow[] MapRecords(IDataReader reader, int startIndex, int length, ref int totalRecordCount)		{
			if (0 > startIndex)
			...
			while (reader.Read())
			{
				...
				if (ri > 0 && ri <= length)
				{
					LawyerContractRow record = new LawyerContractRow();
					recordList.Add(record);
			...
			return (LawyerContractRow[])(recordList.ToArray(typeof(LawyerContractRow)));
		}
