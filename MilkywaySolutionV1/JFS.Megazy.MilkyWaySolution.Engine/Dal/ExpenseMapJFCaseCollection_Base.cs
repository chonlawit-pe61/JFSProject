...
{
	[Serializable]
	public partial class ExpenseMapJFCaseCollection_Base : MarshalByRefObject
	{
		...
		public ExpenseMapJFCaseCollection_Base(int intProcessID)
		{
		...
		public virtual ExpenseMapJFCaseRow[] GetAll()
		{
		...
		{
			AntiSqlInjection.CheckInput(whereSql);
			string sql = "DELETE FROM [dbo].[ExpenseMapJFCase]";
			if (null != whereSql && 0 < whereSql.Length)
			sql += " WHERE " + whereSql;
			return CreateCommand(sql);
		}
		...
		public virtual ExpenseMapJFCaseRow[] GetByExpenseID(int expenseID)
		{
		...
		public virtual ExpenseMapJFCaseRow[] GetByJFCaseTypeID(int jFCaseTypeID)
		{
		...
		public ExpenseMapJFCaseRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql)
		{
			int totalRecordCount = -1;
			return GetAsArray(sqlParameter, whereSql, orderBySql, 0, int.MaxValue, ref totalRecordCount);
		}
		public virtual ExpenseMapJFCaseRow[] GetAsArray(List<SqlParameter> sqlParameter, string whereSql, string orderBySql, int startIndex, int length, ref int totalRecordCount)
		{
		...
		/// <summary>
		/// Gets an array of <see cref="ExpenseMapJFCaseRow"/> objects that
		...
		/// <returns>An array of <see cref="ExpenseMapJFCaseRow"/> objects.</returns>
		public virtual ExpenseMapJFCaseRow[] GetTopAsArray(int top,List<SqlParameter> sqlParameter,string whereSql, string orderBySql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			if (top <= 0)
				top = 1;
			string sql = string.Format("SELECT TOP ({0}) * FROM [dbo].[ExpenseMapJFCase]", top);
			if (null != whereSql && 0 < whereSql.Length)
				sql += " WHERE " + whereSql;
			if (null != orderBySql && 0 < orderBySql.Length)
				sql += " ORDER BY " + orderBySql;
		...
		public ExpenseMapJFCaseRow GetRow(List<SqlParameter> sqlParameter,string whereSql)
		{
			AntiSqlInjection.CheckInput(whereSql);
			int totalRecordCount = -1;
			ExpenseMapJFCaseRow[] rows = null;
		...
		/// <returns></returns>
		public DataTable GetExpenseMapJFCasePagingAsDataTable(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "ExpenseID")
		{
			AntiSqlInjection.CheckInput(whereSql);
		if (startRowIndex < 0)
			startRowIndex = 0;
		...
		string sql = "SELECT COUNT(ExpenseID) AS TotalRow FROM [dbo].[ExpenseMapJFCase] " + whereCount;
		SqlCommand command = CreateCommand(sql);
		command.Parameters.AddRange(sqlParameter.ToArray());
		...
		sql = " SELECT RowRank,ExpenseID,JFCaseTypeID,PriceThreshold,IsOther,SortOrder,ModifiedDate," + totalRow + " AS totalRow," + totalPage + " AS totalPage " +
		" FROM (SELECT *, " +
		" ROW_NUMBER() OVER(ORDER BY " + orderBySql + ") AS RowRank " +
		" FROM [dbo].[ExpenseMapJFCase] " + whereSql +
		" ) AS AllWithRowRank " +
		...
		/// <param name="orderBySql"></param>
		/// <returns></returns>
		public ExpenseMapJFCaseItemsPaging GetExpenseMapJFCasePagingAsArray(List<SqlParameter> sqlParameter,string whereSql, int startRowIndex, int rowPerPage, string orderBySql = "ExpenseID")
		{
		ExpenseMapJFCaseItemsPaging obj = new ExpenseMapJFCaseItemsPaging();
		DataTable dt = GetExpenseMapJFCasePagingAsDataTable(sqlParameter,whereSql,startRowIndex,rowPerPage,orderBySql);
		if(dt.Rows.Count !=0){
			obj.totalRow = Convert.ToInt32(dt.Rows[0]["totalRow"]);
			obj.totalPage = Convert.ToInt32(dt.Rows[0]["totalPage"]);
		}
		ExpenseMapJFCaseItems record;
		ArrayList recordList = new ArrayList();
		for (int i = 0; i < dt.Rows.Count; i++)
		{
			record = new ExpenseMapJFCaseItems();
		...
		obj.expenseMapJFCaseItems = (ExpenseMapJFCaseItems[])(recordList.ToArray(typeof(ExpenseMapJFCaseItems)));
		return obj;
		}
		public ExpenseMapJFCaseRow GetByPrimaryKey(int expenseID, int jFCaseTypeID)
		{
			string whereSql = "[ExpenseID]=" + CreateSqlParameterName("ExpenseID") + " AND [JFCaseTypeID]=" + CreateSqlParameterName("JFCaseTypeID");
			IDbCommand cmd = CreateGetCommand(whereSql, null);
			AddParameter(cmd, "ExpenseID", expenseID);
			AddParameter(cmd, "JFCaseTypeID", jFCaseTypeID);
			ExpenseMapJFCaseRow[] tempArray = MapRecords(cmd);
			return 0 == tempArray.Length ? null : tempArray[0];
		}
		public void Insert(ExpenseMapJFCaseRow value)		{
			string sqlStr = "INSERT INTO [dbo].[ExpenseMapJFCase] (" +
			"[ExpenseID], " + 
			"[JFCaseTypeID], " + 
			"[PriceThreshold], " + 
			"[IsOther], " + 
		...
		public void InsertOnlyPlainText(ExpenseMapJFCaseRow value)		{
			string sqlStr = "INSERT INTO [dbo].[ExpenseMapJFCase] (" +
			"[ExpenseID], " + 
			"[JFCaseTypeID], " + 
			"[PriceThreshold], " + 
		...
		public bool Update(ExpenseMapJFCaseRow value)
		{
			IDbCommand cmd = null;
			if ( value._IsSetExpenseID == true && value._IsSetJFCaseTypeID == true )
			{
				...
				if (strUpdate != string.Empty)
				{
					...
					strUpdate = "UPDATE [dbo].[ExpenseMapJFCase] SET " + strUpdate;
					strUpdate += " WHERE ";
		...
		public bool UpdateOnlyPlainText(ExpenseMapJFCaseRow value)
		{
			IDbCommand cmd = null;
			if ( value._IsSetExpenseID == true && value._IsSetJFCaseTypeID == true )
			{
		...
		protected ExpenseMapJFCaseRow[] MapRecords(IDbCommand command)
		{
			using (IDataReader reader = ExecuteReader(command))
			{
		...
		protected ExpenseMapJFCaseRow[] MapRecords(IDataReader reader)		{
			int totalRecordCount = -1;
			return MapRecords(reader, 0, int.MaxValue, ref totalRecordCount);
		}
		protected ExpenseMapJFCaseRow[] MapRecords(IDataReader reader, int startIndex, int length, ref int totalRecordCount)		{
			if (0 > startIndex)
			...
			while (reader.Read())
			{
				...
				if (ri > 0 && ri <= length)
				{
					ExpenseMapJFCaseRow record = new ExpenseMapJFCaseRow();
					recordList.Add(record);
			...
			return (ExpenseMapJFCaseRow[])(recordList.ToArray(typeof(ExpenseMapJFCaseRow)));
		}
