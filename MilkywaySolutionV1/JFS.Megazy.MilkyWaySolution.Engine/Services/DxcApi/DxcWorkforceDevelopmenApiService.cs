...
namespace JFS.Megazy.MilkyWaySolution.Engine.Services.DxcApi
{
    ...
    public class DxcWorkforceDevelopmenApiService
    {
        private readonly HttpClient _httpClient;
        public DxcWorkforceDevelopmenApiService() : this(null) { }
        public DxcWorkforceDevelopmenApiService(HttpClient httpClient)
        {
            _httpClient = httpClient ?? new HttpClient();
        }
        public async Task<WorkforceDevelopmentResult> GetAsync(string accessToken, string cardID, DopaFilters filters)
        {
            if (filters == null)
            {
            ...
            requestUrl.Append($"https://api.dxc.go.th/services/dxc-qm-moi-linkage-service/api/v2/moi-linkage/persons/{cardID}/mol-dsd-workforce-developments");
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                return new WorkforceDevelopmentResult
                {
            ...
            var resData = JsonConvert.DeserializeObject<WorkforceDevelopmentData>(content);
            return new WorkforceDevelopmentResult
            {
                Status = response.StatusCode.ToString(),
                Result = resData
