...
{
    public class ThaiFiscalYearRegionReport : ApplicantReport
    {
        public override bool Export()
        {
            throw new NotImplementedException();
        }
        public override DataTable GetAsDataTable(ComponentReportFilter filter, string[] columName
            , int startRowIndex, int rowPerPage = 50000, string sortBy = "ProvinceTypeName ASC")
        {
            ...
            string sqlTable = " SELECT  ProvinceTypeName,ThaiFiscalYear" + selectColumn + ",COUNT(ApplicantID) AS NumberOfApplicant" +
                          " FROM jfs_udf_report()" + whereSql +
                          " GROUP BY ProvinceTypeName,ThaiFiscalYear" + selectColumn;
        ...
        public override DataTable GetAllAsDataTable(ComponentReportFilter filter, string[] columName, string sortBy = "")
        {
            ...
            string sql = " SELECT TOP 50000 ProvinceTypeName,ThaiFiscalYear" + selectColumn + ",COUNT(ApplicantID) AS NumberOfApplicant" +
                          " FROM jfs_udf_report()" + whereSql +
                          "  GROUP BY ProvinceTypeName,ThaiFiscalYear" + selectColumn + OrderBySql;
            AntiSqlInjection.CheckInput(sql);
            SqlCommand command = CreateCommand(sql);
            command.Parameters.AddRange(sqlParameter.ToArray());
            DataTable dt = CreateDataTable(command);
