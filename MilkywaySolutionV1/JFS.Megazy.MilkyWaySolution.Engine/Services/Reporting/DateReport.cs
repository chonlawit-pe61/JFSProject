...
{
    public class DateReport : CaseReport
    {
        ...
        public override DataTable GetAsDataTable(ComponentReportFilter filter, string[] columName, int startRowIndex, int rowPerPage = 50000, string sortBy = "RegisterDate DESC")
        {
            ...
            string sqlTable = " SELECT  RegisterDate" + selectColumn +",COUNT(CaseID) AS NumberOfCase" +
                          " FROM jfs_udf_report()" + whereSql +
                          " GROUP BY RegisterDate" + selectColumn;
            string sql = "SELECT COUNT(RegisterDate) AS TotalRow FROM (" + sqlTable + ") Tb ";
        ...
        public override DataTable GetAllAsDataTable(ComponentReportFilter filter, string[] columName, string sortBy = "")
        {
            ...
            string sql = " SELECT TOP 50000 RegisterDate" + selectColumn + ",COUNT(CaseID) AS NumberOfCase" +
                          " FROM jfs_udf_report()" + whereSql +
                          "  GROUP BY RegisterDate" + selectColumn + OrderBySql;
            AntiSqlInjection.CheckInput(sql);
            SqlCommand command = CreateCommand(sql);
            command.Parameters.AddRange(sqlParameter.ToArray());
            DataTable dt = CreateDataTable(command);
