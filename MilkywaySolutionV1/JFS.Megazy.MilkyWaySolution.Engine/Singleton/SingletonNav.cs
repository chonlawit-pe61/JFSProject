...
using JFS.Megazy.MilkyWaySolution.Engine.DataRepository.Navigation;
    ...
    public class SingletonNavi : IDisposable
    {
        ...
        private List<NavItem> GetNavi()
        {
            ...
            try
            {
                ...
                {
                    if (JFS.Megazy.MilkyWaySolution.Engine.Helpers.FileHelper.GetLastUpdateTotalMinutes(path) > 120)
                    {
                        res = GetMenuFile(path);
                    }
                    else
